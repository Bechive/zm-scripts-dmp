...
function __init__()
{
	clientfield::register( "vehicle", "toggle_lensflare", VERSION_SHIP, 1, "int" );
...
function _get_default_target_offset( e_target, n_index )
{
	...
	if ( s_turret.str_weapon_type == "bullet" )
	{
		if ( isdefined( e_target ) )
		{
			...
			{
				z_offset = ( IsVehicle( e_target ) ? 0 : VAL( s_turret.n_torso_targetting_offset,  0 ) );	// if both sentients, the code will handle the offset intelligently
			}
			...
			else if ( IS_EQUAL( e_target.type, "robot" ) )
			{
	...
	if ( VAL( s_turret.n_target_leading_factor, 0 ) != 0 && isdefined( e_target ) && IsSentient( self ) && IsSentient( e_target ) && !IsVehicle( e_target ) )
	{
...
{
	if(isDefined(_get_turret_data( n_index ).e_target) && IS_TRUE(_get_turret_data( n_index ).e_target.ignoreme))
	{
		clear_target( n_index );
	}
...
function _waittill_turret_on_target( e_target, n_index )
{
	do
	{
		wait VAL( self.waittill_turret_on_target_delay, 0.5 );
		if ( !isdefined( n_index ) || ( n_index == 0 ) )
		{
...
function _has_nearby_player_enemy( index, turret )
{
	...
	if ( time >= turret.next_nearby_enemy_time )
	{
		...
		foreach ( player in players )
		{
			...
			if ( Abs( ai_user.origin[2] - player.origin[2] ) <= NEARBY_DISTANCE_Z &&
				Distance2DSquared( ai_user.origin, player.origin ) <= NEARBY_DISTANCE_SQ )
			{
		...
		turret.next_nearby_enemy_time = time + NEARBY_ENEMY_TIME;
	}
...
function _drop_turret( n_index, bExitIfAutomatedOnly )
{
	...
	if ( IsAlive( ai_user ) && (IS_TRUE( ai_user.turret_auto_use ) || (isdefined(bExitIfAutomatedOnly) && !bExitIfAutomatedOnly)) )
...
function _turret_new_user_think( n_index )
{
	...
	while ( true )
	{
		...
		if ( does_have_target( n_index ) && !_user_check( n_index ) && IS_TRUE( self.script_auto_use ) )
		{
...
function _get_potential_targets( n_index )
{
	...
	if ( self.use_non_teambased_enemy_selection === true && !level.teambased )
	{
		...
		for ( i = 0; i < a_all_targets.size; i++ )
		{
			...
			if( !IS_TRUE( level.team_free_targeting ) && ( e_target.team == "free" ) )
				continue;
	...
	else if ( isdefined( str_team ) )
	{
		...
		if ( _has_target_flags( TURRET_TARGET_AI, n_index ) )
		{
			...
			if( IS_TRUE( level.team_free_targeting ) )
			{
...
//			a_potential_targets = ArrayCombine( a_potential_targets, a_drone_targets, true, false );
	...
	if ( isdefined( str_team ) )
	{
		...
		for ( i = 0; i < a_potential_targets.size; i++ )
		{
			...
			if ( IS_TRUE( e_target.ignoreme ) || !isdefined( e_target.health ) || ( e_target.health <= 0 ) )
			{
				ignore_target = true;
			}
...
function _is_target_within_range( e_target, s_turret )
{
	if ( isdefined( s_turret.n_max_target_distance_squared ) || isdefined( s_turret.n_min_target_distance_squared ) )
	{
		...
		if ( n_dist_squared > VAL( s_turret.n_max_target_distance_squared, 811711611 ) )
			return false;

		if ( n_dist_squared < VAL( s_turret.n_min_target_distance_squared, 0 ) )
			return false;
	}
...
function can_hit_target( e_target, n_index )
{
	...
	if ( b_target_in_view )
	{
		...
		{
			b_trace_passed = trace_test( e_target, v_offset - ( 0, 0, ( IsVehicle( e_target ) ? 0 : VAL( s_turret.n_torso_targetting_offset, 0 ) ) ), n_index );
		}
