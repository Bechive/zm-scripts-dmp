...
#define TROPHY_DISABLE_LIMIT 4 //Number of times that a player can destroy the trophy weakspot before it is permanently destroyed
...
function quadtank_weapon_think_cannon()
{
	...
	while ( 1 )
	{
		if( self.hold_cannon === true || !vehicle_ai::IsCooldownReady( "main_cannon" ) )
		{
...
function QuadtankCallback_VehicleDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, weapon, vPoint, vDir, sHitLoc, vDamageOrigin, psOffsetTime, damageFromUnderneath, modelIndex, partName, vSurfaceNormal )
{
	...
	maxDamage = self.healthdefault * ( 0.20 - 0.025 * num_players );
	...
	{	
		if( self.damage_during_trophy_down + iDamage > self.settings.trophysystemdisablethreshold && self.trophy_disables < TROPHY_DISABLE_LIMIT && !isdefined(driver) )
		{
	...
	{
		//self notify( "pain" ); // too many pains. no pain for damage state change
		playsoundatposition ("prj_quad_impact", self.origin);
	}
