...
function statusDialog( dialog, team, objectiveKey )
{
	...
	if ( isdefined( dialogTime) )
	{
		...
		game["dialogTime"][dialog] = time + DIALOG_THROTTLE;
	}
...
function onUseWithNeutralizingFlag( player )
{
	...
	if ( oldTeam == "neutral" )
	{
		...
		string = GetDomFlagUseString( label, false );
		level.bestSpawnFlag[ oldTeam ] = self.levelFlag;
	...
	else
	{
		...
		string = GetDomFlagUseString( label, true );
	...
	if( oldTeam == "neutral" )
	{
		thread give_capture_credit( touchList, string, oldTeam, isBFlag, true );
	}
	else
	{
...
function onUseWithoutNeutralizingFlag( player )
{
	...
	thread give_capture_credit( touchList, string, oldTeam, isBFlag, false );
...
function updateDomScores()
{
	...
	while ( !level.gameEnded )
	{
		...
		if( ( totalFlags == 3 ) && ( game["teamScores"]["allies"] == round_score_limit - 1 ) && ( game["teamScores"]["axis"] == round_score_limit - 1 ) )
			level.clampScoreLimit = false; // clear the clamp so we dont end up tie in the situation wehre the score is 199 vs 199 and one team has 2 flags and the other one has 1
		...
		if ( warningScore && !playedNearEndVo )
		{
			...
			if ( isdefined( winningTeam ) )
			{
				...
				{
					if ( RandomInt( 4 ) < 3 ) // 1 in winning, 3 in winningFinal
					{
						nearWinning = "nearWinningFinal";
					}
					
					if ( RandomInt( 4 ) < 1 ) // 3 in losing, 1 in losingFinal
					{
						nearLosing = "nearLosingFinal";
					}
				}
		...
		if ( (((timePassed / 1000) > 120 && numOwnedFlags < 2) || ((timePassed / 1000) > 300 && numOwnedFlags < 3)) && ( GameModeIsMode( GAMEMODE_PUBLIC_MATCH ) ) )
		{
