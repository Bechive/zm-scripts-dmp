...
function main()
{
	...
	clientfield::register( "world", "hardpoint", VERSION_SHIP, 5, "int" );
	clientfield::register( "world", "hardpointteam", VERSION_SHIP, 5, "int" );
...
function KothMainLoop()
{
	...
	while( 1 )
	{
		...
		{
			level.zone.gameobject gameobjects::set_visible_team( "any" );
			level.zone.gameobject gameobjects::set_flags( OBJECTIVE_FLAG_TARGET );
			...
			level.zone.gameobject gameobjects::set_flags( OBJECTIVE_FLAG_NORMAL );
...
function onPlayerKilled( eInflictor, attacker, iDamage, sMeansOfDeath, weapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	if ( medalGiven == true )
	{
		if ( IS_TRUE( level.zone.gameobject.isContested ) )
		{
