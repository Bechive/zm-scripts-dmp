...
function Callback_VehicleDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, weapon, vPoint, vDir, sHitLoc, vDamageOrigin, psOffsetTime, damageFromUnderneath, modelIndex, partName, vSurfaceNormal )
{
	...
	if( !(iDFlags & IDFLAGS_NO_PROTECTION) )
	{
		...
		{
			// do not use level.friendlyfire per design as it allows for griefing
			// we still need to ask though so that killstreaks can self destruct as needed
		...
		if ( eAttackerNotSelf && ( eAttackerIsNotOwner || ( isdefined( self.selfDestruct ) && !self.selfDestruct ) || ( self.forceDamageFeedback === true ) ) )
		{
