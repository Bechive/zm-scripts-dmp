...
class cDoor
{
	...
	function init_hint_trigger()
	{
		...
		//
		if ( m_s_bundle.door_unlock_method == "default" && !IS_TRUE(m_s_bundle.door_trigger_at_target) )
		{
			return;
		}
	...
	function close_internal()
	{
		...
		if( IS_TRUE(m_n_door_connect_paths) )
		{
			m_e_door DisconnectPaths();
		}
	...
	{		
		...
		{
			if ( !IS_TRUE(m_s_bundle.door_closes) )
			{
...
//				m_e_trigger SetHintString( m_s_bundle.door_open_message );
			...
			else if (IS_TRUE(m_s_bundle.door_use_hold))
			{
...
//				m_e_trigger SetHintString( "Hold ^3[{+activate}]^7 to open door" );
	...
	{
		str_hint = "";
		if ( IS_TRUE( m_s_bundle.door_trigger_at_target ) )
		{
...
REGISTER_SYSTEM( "doors", &__init__, undefined )
...
function setup_door_scriptbundle( s_door_bundle, s_door_instance )
{
	...
	if( IS_TRUE(c_door.m_s_bundle.door_connect_paths) )
	{
...
function door_update( c_door )
{
	...
	if ( IS_TRUE( c_door.m_s_bundle.door_locked ) && str_unlock_method != "key" )
	{
...
function door_wait_until_clear( c_door, e_triggerer )
{
	...
	if ( IS_TRUE( c_door.m_s_bundle.door_trigger_at_target ) )
	{		
