...
REGISTER_SYSTEM( "vehicle_ai", &__init__, undefined )
...
function owner_in_line_of_fire()
{
	...
	gun_angles = self GetTagAngles( VAL( self.avoid_shooting_owner_ref_tag , "tag_flash" ) );
...
function Javelin_LoseTargetAtRightTime( target )
{
	...
	{
		if( proj GetVelocity()[2] < -150 && DistanceSquared( proj.origin, target.origin ) < SQR(1200) )
		{
...
function default_death( params )
{
	...
	waittime = VAL( self.waittime_before_delete, 0 );
...
function defaultstate_death_update( params )
{
	...
	{
		death_type = VAL( get_death_type( params ), "default" );
...
function try_detonate( closest, attacker )
{
	...
	{
		if( distanceSquared( closest.origin, self.origin ) < SQR( 80 ) )
		{
...
function FindNewPosition( sight_check_height )
{
	...
	queryResult = PositionQuery_Source_Navigation( self.origin, 0, POINTS_MAX_DIST, 300/*half height*/, point_spacing, self, point_spacing * 2 );
	PixEndEvent();
	...
	if ( isdefined( self.enemy ) )
	{
		...
		{	
			...
			{
				ADD_POINT_SCORE( point, "tooCloseToSelf", -170 );
			}
			
			if( isdefined( point.sight ) && point.sight )
			{
				ADD_POINT_SCORE( point, "sight", 250 );
			}
			if( isdefined( point.sight2 ) && point.sight2 )
			{
				ADD_POINT_SCORE( point, "sight2", 150 );
			}
			if( isdefined( point.sight3 ) && point.sight3 )
			{
				ADD_POINT_SCORE( point, "sight3", 150 );
			}
...
function PositionQuery_Filter_Random( queryResult, min, max )
{
	foreach( point in queryResult.data )
	{
		...
		ADD_POINT_SCORE( point, "random", score );
	}
}
...
function PositionQuery_Filter_OutOfGoalAnchor( queryResult, tolerance = 1 )
{
	foreach( point in queryResult.data )
	{
		if ( point.distToGoal > tolerance )
		{
			...
			ADD_POINT_SCORE( point, "outOfGoalAnchor", score );
		}
	}
}
...
function PositionQuery_Filter_EngagementDist( queryResult, enemy, engagementDistanceMin, engagementDistanceMax )
{
	...
	enemy_origin = FLAT_ORIGIN( enemy.origin );

	vec_enemy_to_self = VectorNormalize( FLAT_ORIGIN( self.origin ) - enemy_origin );
		
	foreach( point in queryResult.data )
	{
		point.distAwayFromEngagementArea = 0;	// <-------  result value initialization
	
		vec_enemy_to_point = FLAT_ORIGIN( point.origin ) - enemy_origin;
...
function PositionQuery_Filter_DistAwayFromTarget( queryResult, targetArray, distance, tooClosePenalty )
{
	...
	foreach( point in queryResult.data )
	{
		...
		if ( tooClose )
		{
			ADD_POINT_SCORE( point, "TooCloseToOthers", tooClosePenalty );
		}
	}
}
