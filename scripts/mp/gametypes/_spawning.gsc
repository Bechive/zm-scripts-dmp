...
function __init__()
{
	...
	level.spawnProtectionTimeMS = int ( VAL( level.spawnProtectionTime, 0 ) * 1000 );
...
function updateAllSpawnPoints()
{
	...
	clearSpawnPoints( SPL_NORMAL );
	
	if ( level.teambased )
	{
		foreach( team in level.teams )
		{ 
			addSpawnPoints( team, level.player_spawn_points[ team ], SPL_NORMAL );
			EnableSpawnPointList( SPL_NORMAL, util::getTeamMask( team ) );
		}
	}
	else
	{
		foreach( team in level.teams )
		{ 
			addSpawnPoints( "free", level.player_spawn_points[ team ], SPL_NORMAL );
			EnableSpawnPointList( SPL_NORMAL, util::getTeamMask( team ) );
		}
	}
...
function update_fallback_spawnpoints()
{
	...
	if ( level.teambased )
	{
		foreach( team in level.teams )
		{ 
			addSpawnPoints( team, level.player_fallback_points[ team ], SPL_FALLBACK );
		}
	}
	else
	{
		foreach( team in level.teams )
		{ 
			addSpawnPoints( "free", level.player_fallback_points[ team ], SPL_FALLBACK );
		}
	}
}
...
function add_fallback_spawnpoints( team, point_class )
{
	...
	DisableSpawnPointList( SPL_FALLBACK, util::getTeamMask( team ) );
}

function is_spawn_trapped( team )
{
	// this feature is only on for public match
...
function use_start_spawns(predictedSpawn)
{
	...
	// if we have more players then start spawns dont use them
	if( level.teambased )
	{
	...
	predictedSpawn )
{
	...
	if ( spawn_trapped )
	{
		EnableSpawnPointList( SPL_FALLBACK, util::getTeamMask( point_team ) );
	}
	else
	{
		DisableSpawnPointList( SPL_FALLBACK, util::getTeamMask( point_team ) );
	}
...
function initialSpawnProtection()
{
	...
	self clientfield::set( CLIENT_FIELD_KILLSTREAK_SPAWN_PROTECTION, 1 );
	self.ignoreme = true;
	wait level.spawnProtectionTime;
	self clientfield::set( CLIENT_FIELD_KILLSTREAK_SPAWN_PROTECTION, 0 );
