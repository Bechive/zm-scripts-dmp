...
REGISTER_SYSTEM_EX( "spawner", &__init__, &__main__, undefined )
...
{
	ARRAY_ADD( level.ai[ self.team ], self );
...
//	}
	...
	else if ( IS_EQUAL( self.weaponclass,"gas" ) )
	{
...
//	}
	...
	if ( IS_TRUE( level.using_awareness ) )
	{
		return;
	}
	
	if ( IS_ARTILLERY( self ) )//we don't want to check this for turrets
	{
		return;	
	}
...
function go_to_spawner_target(target_names)
{
	...
	for ( i = 0; i < target_names.size; i++ )
	{
		...
		foreach ( node in target_nodes )
		{
			...
			else if ( SPAWNFLAG( node, SPAWNFLAG_PATH_DISABLED ) )
			{
				ARRAY_ADD( a_nodes_unavailable, node );
			}
			else
		...
		while ( nodes.size == 0 )
		{
			foreach ( node in a_nodes_unavailable )
			{
				if ( !IsNodeOccupied( node )
				    ...
				    && !SPAWNFLAG( node, SPAWNFLAG_PATH_DISABLED ) )
				{
					ARRAY_ADD( nodes, node );
					break;
				}
...
function fallback_spawner_think( num, node_array, ignoreWhileFallingBack )
{
	...
	while( self.count > 0 )
	{
		...
		WAIT_SERVER_FRAME; // Wait until he does all his usual spawned logic so he will run to his node
...
{
	Assert( SPAWNFLAG( self, SPAWNFLAG_ACTOR_SPAWNER ), "Spawner at origin" + self.origin + "/" +( self GetOrigin() ) + " is not a spawner!" );
}
...
function spawn( b_force = false, str_targetname, v_origin, v_angles, bIgnoreSpawningLimit )
{
	...
	while ( true )
	{
		if ( !IS_TRUE( bIgnoreSpawningLimit ) && !IS_TRUE( self.ignorespawninglimit ) ) //TU1 - self.ignorespawninglimit - only to be used when nothing else will suffice
		{
			global_spawn_throttle( MAX_SPAWNED_PER_FRAME );
		}
		
		if( IS_BONUSZM && !IsDefined( self ) )
		{
			// sjakatdar TU2 (10/29/2015) In rare cases, the spawner might get deleted which might lead to un-recoverable script errors.
			return;
		}
	...
	{
		if ( SPAWNFLAG( self, SPAWNFLAG_ACTOR_SCRIPTMAKEROOM ) )
		{
			makeroom = true;
		}
	}
	...
	{
		if ( SPAWNFLAG( self, SPAWNFLAG_VEHICLE_SCRIPTMAKEROOM ) )
		{
			makeroom = true;
		}
	}
	...
	if ( b_force || SPAWNFLAG( self, SPAWNFLAG_ACTOR_SCRIPTFORCESPAWN ) || isdefined( self.script_forcespawn ) )
	{
		force_spawn = true;
	}
		
	if ( SPAWNFLAG( self, SPAWNFLAG_ACTOR_SCRIPTINFINITESPAWN ) )
	{
		infinitespawn = true;
	}
	...
	if ( ( deleteonzerocount || IS_TRUE( self.script_delete_on_zero ) ) && isdefined( self.count ) && ( self.count <= 0 ) )
	{
		self Delete();
	}
...
{
	DEFAULT( v_origin, self.origin );
	DEFAULT( v_angles, self.angles );
	
	if ( IsActor( self ) )
	{
		self ForceTeleport( v_origin, v_angles, true, b_reset_entity );
	}
	else
	{
...
function check_player_requirements()
{
	...
	// TODO: depricated script_numplayers, remove at some point
...
function add_global_spawn_function( team, spawn_func, param1, param2, param3, param4, param5 )
{
	...
	ARRAY_ADD( level.spawn_funcs[ team ], func );
}
