...
REGISTER_SYSTEM( "trigger", &__init__, undefined )
...
function __init__()
{
	...
	{
		if ( SPAWNFLAG( trig, SPAWNFLAG_TRIGGER_LOOK ) )
		{
	...
	foreach ( trig in get_all() )
	{
		if ( trig.classname != "trigger_damage" )
		{
			...
			{
				if ( SPAWNFLAG( trig, SPAWNFLAG_TRIGGER_SPAWN ) )
				{
...
{
	if ( IS_TRUE( self.script_trigger_allplayers )
	    && ( SPAWNFLAG( self, SPAWNFLAG_TRIGGER_AI_AXIS )
	    || SPAWNFLAG( self, SPAWNFLAG_TRIGGER_AI_ALLIES )
	    || SPAWNFLAG( self, SPAWNFLAG_TRIGGER_AI_NEUTRAL )
	    || SPAWNFLAG( self, SPAWNFLAG_TRIGGER_NOT_PLAYER )
	    || SPAWNFLAG( self, SPAWNFLAG_TRIGGER_VEHICLE ) ) )
	{
...
function trigger_look( trigger )
{
	...
	while ( true )
	{
		...
		if ( IsPlayer( e_other ) )
		{
			...
			{
				if ( e_other util::is_looking_at( e_target, trigger.script_dot, IS_TRUE( trigger.script_trace ) )
...
function flag_set_trigger( trigger, str_flag )
{
	...
	DEFAULT( str_flag, trigger.script_flag );
...
function wait_till( str_name, str_key = "targetname", e_entity, b_assert = true )
{
	if ( isdefined( str_name ) )
	{
		...
		if( IS_BONUSZM )
		{
			// sjakatdar ( 09/21/2015 ) - A temporary fix which should be removed in T8 branch. In campaign zombies,
			// players can double jump ahead and hit the trigger before trigger::wait_till is called, which can end up in this assert.
			//Assert( !b_assert || ( triggers.size > 0 ), "trigger not found: " + str_name + " key: " + str_key );
			// Its better to just return true if the trigger is not found instead.
			if( triggers.size <= 0 )
				return;
		}
		else
	...
	else
	{
		...
		{
			// sjakatdar ( 09/21/2015 ) - A temporary fix which should be removed in T8 branch. In campaign zombies,
			// players can double jump ahead and hit the trigger before trigger::wait_till is called, which can end up in script errors
			// Its better to just return true if the trigger is not found instead.
			if( !IsDefined( self ) )
				return;
		}
...
{
	return ( isdefined( trig ) ? SPAWNFLAG( trig, SPAWNFLAG_TRIGGER_LOOK ) && !IS_EQUAL( trig.classname, "trigger_damage" ) : false );
}

function is_trigger_once( trig )
{
	return ( isdefined( trig ) ? SPAWNFLAG( trig, SPAWNFLAG_TRIGGER_TRIGGER_ONCE ) || ( IS_EQUAL( self.classname, "trigger_once" ) ) : false );
}
...
function kill_spawner_trigger( trigger )
{
	...
	{
		if ( IS_EQUAL( ent.classname, "spawn_manager" ) && ( ent != trigger ) )
		{
			ent Delete();
		}
	}
