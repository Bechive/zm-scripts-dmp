...
function watch_usage()
{
	...
	{	
		...
		if( isDefined(curWeapon.gadget_type) && curWeapon.gadget_type == GADGET_TYPE_HERO_WEAPON )
		{
...
function track_fire( curWeapon )
{
	...
	self TrackWeaponFireNative( curWeapon, TRACK_WEAPON_SHOT_FIRED, self.hits, 1, self.class_num, weaponPickedUp, self.primaryLoadoutGunSmithVariantIndex, self.secondaryLoadoutGunSmithVariantIndex );
	// old:
...
function begin_grenade_tracking()
{
	...
	if( IS_BONUSZM || IS_TRUE( level.projectiles_should_ignore_world_pause ) )
	{
